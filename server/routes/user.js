...
const config = require("../mariadb_config.js");
const conn = mysql.createConnection(config);

// web3설정
const Web3 = require("../../node_modules/web3");
const Accounts = require("../../node_modules/web3-eth-accounts");
...
async function init() {
    await hp3.eth.net.getId((err, result) => {
        ...
        const _meta = new hp3.eth.Contract(SimpleToken.abi, deployedNetwork.address);
        meta = _meta;
    });
...
const { func } = require("../../node_modules/prop-types");
const saltRounds = 10;
...
router.post("/signUp", async (req, res, next) => {
    ...
    let sql = "INSERT INTO user(userid,userpw,useremail,userpbk,userpvk,joindate) VALUES (?,?,?,?,?,NOW())";
                        ...
                        if (isMatch) {
                            ...
                            conn.query(sql2, [token, id], function (err, result) {
                                if (err) res.status(400).send(err);
                                    ...
                                    res.cookie("x_auth", token).status(200).json({ success: true, userId: token });
                                }
                            });
                        } else {
                            res.json({ success: false });
